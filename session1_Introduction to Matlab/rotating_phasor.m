...
% x = cos(pi/2*t).*exp(j*2*pi*f*t);		%unit amplitude phasor
x =exp(t).*exp(j*2*pi*f*t);
xr = real(x);               %Real part
...
plot(xr(1:ii),t(1:ii),'r');             % plot real component against time
hold on;
plot([xr(ii),xr(ii)],[t(1),t(ii)],'b') % plot horizontal line indicating current value of xi
hold off
